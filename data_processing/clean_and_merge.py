...
def process_data():
    ...
    # Count daily crimes
    daily_crimes = (
        crime_df.groupby("date_only").size().reset_index(name="total_violent_crimes")
    )

    # Crimes within 3km of soldier field
    stadium_crimes = crime_df[crime_df["dist_to_stadium_km"] <= 3.0]
    daily_stadium_crimes = (
        stadium_crimes.groupby("date_only").size().reset_index(name="stadium_crimes")
    )

    daily_merged = pd.merge(
        daily_crimes, daily_stadium_crimes, on="date_only", how="left"
    )
    ...
    seasonal_old = (
        final_daily.groupby("year").agg({"stadium_crimes": "sum"}).reset_index()
    )
    ...
    crime_totals_long = crime_totals_df.melt(
        id_vars="Series", var_name="month_year", value_name="total_crimes"
    )
    # Extract year
    crime_totals_long["year"] = (
        crime_totals_long["month_year"].str.split("-").str[1].astype(int)
    )
    # Convert total_crimes to numeric, filling NaNs with 0
    crime_totals_long["total_crimes"] = pd.to_numeric(
        crime_totals_long["total_crimes"], errors="coerce"
    ).fillna(0)
    ...
    seasonal = pd.merge(
        seasonal_merged, pfr_df, left_on="year", right_on="Year", how="right"
    )
    ...
    heatmap_data = crime_df[crime_df["date_only"].dt.date.isin(game_dates)][
        ["latitude", "longitude", "primary_type", "dist_to_stadium_km"]
    ]
