...
def calculate_macro_trends():
    ...
    if not game_day_totals: return None
...
def create_line_chart(df, game_year):
    ...
    fig.update_layout(
        xaxis_title="Hours Relative to Kickoff", yaxis_title="Number of Crimes",
        hovermode="x unified",
...
def create_heatmap(df_g, df_h, show_h):
    ...
    layers.append(pdk.Layer(
        "ScatterplotLayer", data=stadium_data,
        get_position=["lon", "lat"], get_color=[255, 100, 0, 200], get_radius=800,
    ))

    view_state = pdk.ViewState(longitude=SOLDIER_FIELD_LON, latitude=SOLDIER_FIELD_LAT, zoom=10.5, pitch=0)
    return pdk.Deck(layers=layers, initial_view_state=view_state, tooltip={"text": "Crime Density"})
...
with tab2:
    ...
    st.write("Analyzes the 9-hour window across **all** historical games running a paired t-test against baselines.")
    ...
    if macro_results:
        ...
        if p_val < 0.05:
            if t_stat > 0:
                st.error(f"🚨 **Significant Increase:** Crime is definitively **higher** on game days. (p={p_val:.4f})")
            else:
                st.success(f"📉 **Significant Decrease:** Crime is definitively **lower** on game days. (p={p_val:.4f})")
        else:
            st.info(f"📊 **No Significant Difference:** No statistical difference in crime volume. (p={p_val:.4f})")
