...
def load_data():
    ...
    try:
        ...
        heatmap_df = pd.read_csv(
            "data/processed/filtered_jan18_crimes.csv"
...
def create_heatmap(heatmap_df):
    ...
    layer = pdk.Layer(
        "HeatmapLayer",
        heatmap_df[['latitude', 'longitude']].dropna().to_dict(orient='records'),
    ...
    stadium_layer = pdk.Layer(
        "ScatterplotLayer",
        [{"lat": SOLDIER_FIELD_LAT, "lon": SOLDIER_FIELD_LON}],
...
def main():
    ...
    st.markdown(
        "Do seasons with poorly performing Bears teams correlate with higher overall violent crime rates?"
    )

    col1, col2 = st.columns([2, 1])
    with col1:
        st.plotly_chart(plot_seasonal_trends(pd.read_csv('data/processed/seasonal_df.csv')), width="stretch")
    ...
    with col2:
        ...
        gt_tbl = (
            gt.GT(
                seasonal_df[cols_for_table].sort_values("year", ascending=False).head(5)
            )
    ...
    st.markdown(
        "Where do crimes occur on Game Days specifically? The orange circle represents Soldier Field."
    )
    ...
    if "year" in heatmap_df.columns:
        years = sorted(
            heatmap_df["year"].dropna().unique().astype(int).tolist(), reverse=True
        )
        selected_years = st.multiselect(
            "Toggle Years to Display", options=years, default=years
        )
